...
export function renderDocumentPdfToBuffer(document: unknown): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const worker = new Worker(WORKER_PATH, { workerData: { document } });
    let settled = false;
    ...
    worker.once("message", (msg: { ok: boolean; buffer?: Buffer; error?: string }) => {
      ...
      if (msg.ok && msg.buffer) {
        ...
        reject(new Error(msg.error || "Failed to generate PDF"));
      }
    });
