...
import { useQuote, useCreateQuote, useUpdateQuote, type QuoteFormItem } from "@/hooks/useQuotes";
...
const emptyItem: QuoteFormItem = { description: "", quantity: 1, unitPrice: 0, taxRate: 0, discount: 0 };
...
export default function QuoteForm() {
  ...
  const [items, setItems] = useState<QuoteFormItem[]>([{ ...emptyItem }]);
  const [showNewCustomer, setShowNewCustomer] = useState(false);

  useEffect(() => {
    if (existingQuote) {
      setCustomerId(existingQuote.customer.id);
      setIssueDate(existingQuote.issueDate.slice(0, 10));
      setExpiryDate(existingQuote.expiryDate ? existingQuote.expiryDate.slice(0, 10) : "");
      setCurrency(existingQuote.currency);
      setPoNumber(existingQuote.poNumber || "");
      setNotes(existingQuote.notes || "");
      setTerms(existingQuote.terms || "");
      setInvoiceDiscountType(existingQuote.invoiceDiscountType || "FLAT");
      setInvoiceDiscountValue(existingQuote.invoiceDiscountValue || 0);
      setItems(
  ...
  function updateItem(index: number, field: keyof QuoteFormItem, value: string | number) {
  ...
  return (
    <div>
      <PageHeader title={isEditing ? "Edit Quote" : "Create Quote"} />
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="flex flex-col gap-4">
          ...
          <Card>
            <CardContent className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-end gap-2 border-t border-border pt-3">
                <Label className="mb-0 whitespace-nowrap text-xs text-fg-muted">Quote discount</Label>
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={saving}>
              {saving ? "Saving..." : isEditing ? "Save changes" : "Save Quote"}
            </Button>
          </div>
        </div>
        ...
        <div className="lg:sticky lg:top-6 lg:self-start">
          <InvoicePreview
            ...
            notes={notes}
            terms={terms}
            invoiceNumber={existingQuote?.number}
            poNumber={poNumber}
